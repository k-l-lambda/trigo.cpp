/**
 * Test MCTS from a prefix sequence
 *
 * Usage:
 *   ./test_mcts_from_prefix <board_shape> <moves> <num_simulations> [model_path]
 *
 * Example:
 *   ./test_mcts_from_prefix "5x3x4" "a0z b0z aza azz azy azb a0y a0b 0az yzb 00y yzz" 200
 *
 * This will:
 * 1. Parse the move sequence
 * 2. Replay moves
 * 3. Run MCTS from that position
 * 4. Print detailed statistics (if MCTS_ENABLE_PROFILING is ON)
 *
 * TGN moves format: space-separated list of moves, "Pass" for pass move
 */
...
int main(int argc, char* argv[])
{
	...
	// Initialize AlphaZero MCTS with prefix caching
	std::cout << "=== Initializing CachedMCTS ===" << std::endl;

	// Construct model paths
	std::string prefix_model = model_path + "/base_model_prefix.onnx";
	std::string eval_cached_model = model_path + "/base_model_eval_cached.onnx";
	std::string policy_head = model_path + "/policy_head.onnx";
	std::string value_head = model_path + "/value_head.onnx";

	// Check if model files exist
	std::vector<std::string> required_files = {prefix_model, eval_cached_model, policy_head, value_head};
	...
	try
	{
		...
		// Create CachedMCTS engine
		auto mcts = std::make_shared<CachedMCTS>(
			inferencer,
			num_simulations,
			1.414f,    // c_puct
		...
		// Wrap in policy
		auto policy = std::make_unique<CachedMCTSPolicy>(mcts);

		std::cout << "MCTS initialized with " << num_simulations << " simulations" << std::endl;
		std::cout << "Note: Rebuild with -DMCTS_ENABLE_PROFILING=ON to see detailed statistics" << std::endl;
		std::cout << std::endl;
