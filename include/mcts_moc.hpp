...
#include "mcts.hpp"  // For PolicyAction and MCTSNode
...
/**
 * Pure MCTS Engine (Reference Implementation)
 *
 * Performs Monte Carlo Tree Search with random rollouts
 * NOTE: This is slower than the main MCTS class. For production, use mcts.hpp.
 */
class PureMCTS
{
private:
...
public:
	PureMCTS(int num_sims = 800, float c = 1.414f, int seed = 42)
...
private:
	 ...
	 *
	 * TODO: This is a placeholder implementation using expensive random rollouts.
	 * Performance: ~1000ms per simulation on 5x5x5 board (99.9% of MCTS time).
	 *
	 * PLANNED REPLACEMENT: Use value network inference instead (AlphaZero-style):
	 * - Replace with SharedModelInferencer::value_inference()
	 * - Expected speedup: 100× faster (~1-10ms per evaluation)
